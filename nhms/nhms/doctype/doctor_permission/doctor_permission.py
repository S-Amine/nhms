...
class DoctorPermission(Document):
	def before_save(self):
		...
		frappe.share.add("Patient", self.created_by, user, read=1, write=1, submit=1, share=1, everyone=0, notify=1)
		...
		for bilan in bilans :
			frappe.share.add("Bilan", bilan[0], user, read=1, write=1, submit=1, share=1, everyone=0, notify=1)
	# def on_trash(self):
	# 	user = frappe.get_doc(doctype="User", username=self.doctor_nin)
